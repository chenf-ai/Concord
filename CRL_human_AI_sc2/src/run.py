...
def run_sequential(args, logger):
    ...
    if args.checkpoint_path != "":
        ...
        if args.evaluate or args.save_replay:
            if args.use_Concord:
                mac.set_task_id(0)
            mac.init_human_model('./results/models/offpg__2023-04-23_10-42-25/21052/agent.th')
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.save_model and (runner.best_test_returns > best_test_returns_old):
            print('best_test_return_mean:', runner.best_test_returns)
            save_path = os.path.join(args.local_results_path, "models", args.unique_token, 'best')
            os.makedirs(save_path, exist_ok=True)
            logger.console_logger.info("The new best model is up to {}".format(runner.best_test_returns))
            logger.console_logger.info("Saving models to {}".format(save_path))
            learner.save_models(save_path)
