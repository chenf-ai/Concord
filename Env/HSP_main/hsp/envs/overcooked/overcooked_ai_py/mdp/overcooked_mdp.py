...
from hsp.envs.overcooked.overcooked_ai_py.utils import pos_distance, load_from_json
from hsp.envs.overcooked.overcooked_ai_py.data.layouts import read_layout_dict
...
class OvercookedState(object):
    ...
    def __eq__(self, other):
        ...
        return isinstance(other, OvercookedState) and \
            ...
            order_list_equal and self.timestep == other.timestep
    ...
    def __str__(self):
        return 'Players: {}, Objects: {}, Order list: {} Timestep: {}'.format( 
            str(self.players), str(list(self.objects.values())), str(self.order_list), str(self.timestep))
...
BASE_REW_SHAPING_PARAMS = {
    "PLACEMENT_IN_POT_REW": 3, #3,
    "DISH_PICKUP_REWARD":3,
    "SOUP_PICKUP_REWARD": 5, #5,
    "ONION_PICKUP_REW": 0, #forced取0
    "DISH_DISP_DISTANCE_REW": 0,
    "POT_DISTANCE_REW": 0,
    "SOUP_DISTANCE_REW": 0
}

FANCP_REW_SHAPING_PARAMS = {
    "PLACEMENT_IN_POT_REW": 3, #5,
    "DISH_PICKUP_REWARD":5,    # 3
    "SOUP_PICKUP_REWARD": 7, #10,
    "ONION_PICKUP_REW": 0, #forced取0
    "DISH_DISP_DISTANCE_REW": 0,
    "POT_DISTANCE_REW": 0,
    "SOUP_DISTANCE_REW": 0
...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_order_list=None, cook_time=20, num_items_for_soup=3, delivery_reward=20, rew_shaping_params=None, layout_name="unnamed_layout", partial_obs=False):
        """
        ...
        for y in range(len(self.terrain_mtx)):
            for x in range(len(self.terrain_mtx[0])):
                ...
                if terrain_type == ' ':
                    ...
                    # closet
                    # randomly put onion, dish, soup or empty
                    ...
                    # pot
                    # randomly put empty, soup with 1/2/3 onions
    ...
    def resolve_interacts(self, new_state, joint_action):
        ...
        for player_idx, player, action in zip(range(self.num_players), new_state.players, joint_action):
            ...
            if terrain_type == 'X':
                ...
                shaped_reward[player_idx] += self.reward_shaping_params["ONION_PICKUP_REW"]
    ...
    def lossless_state_encoding(self, overcooked_state, debug=False):
        ...
        def process_for_player(primary_agent_idx):
            ...
            # zpy partial sight 5*5
            if self.partial_obs:
                state_mask_stack = np.array(state_mask_stack).astype(int)
                # print(state_mask_stack.shape)
                primary_pos = overcooked_state.players[primary_agent_idx].position
                partial_state_mask = -1 * np.ones((5, 5, state_mask_stack.shape[2]))
                partial_state_mask = partial_state_mask.astype(int)
                x1 = max(primary_pos[0] - 2, 0)
                y1 = max(primary_pos[1] - 2, 0)
                x2 = min(primary_pos[0] + 2, state_mask_stack.shape[0] - 1)
                y2 = min(primary_pos[1] + 2, state_mask_stack.shape[1] - 1)
                partial_state_mask[x1 - primary_pos[0] + 2 : x2 - primary_pos[0] + 3,
                               y1 - primary_pos[1] + 2 : y2 - primary_pos[1] + 3] \
                        = state_mask_stack[x1:x2 + 1, y1:y2 + 1]
                return partial_state_mask
