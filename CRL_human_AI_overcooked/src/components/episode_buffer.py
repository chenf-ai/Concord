...
class ReplayBuffer(EpisodeBatch):
    ...
    def sample_by_index(self, start_index, batch_size):
        assert self.can_sample(batch_size)
        assert start_index + batch_size <= self.episodes_in_buffer
        return self[start_index : start_index + batch_size]

    def uni_sample_for_recognizer(self, start, range, batch_size):
        assert self.can_sample(batch_size)
        ep_ids = np.random.choice(range, batch_size, replace=False) + start
        return self[ep_ids]
