...
class NonSharedMACHyper:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.recognizer is not None:
            recognizer_inputs = th.cat([agent_Human_inputs[:, :self.args.vecobs_dim], agent_Human_inputs[:, ep_batch['obs'].shape[-1]:]], dim=1)
            predict_embedding, self.hidden_states_rec = self.recognizer(recognizer_inputs, self.hidden_states_rec)
            self.given_embedding = predict_embedding.mean(0).detach().clone()
        ...
        if self.agent_output_type == "pi_logits":
            ...
            if not test_mode:
                ...
                if self.random_action:
                    agent_outs = th.ones_like(agent_outs) / epsilon_action_num
                else:
    ...
    def init_hidden(self, batch_size):
        ...
        if self.recognizer is not None:
            self.hidden_states_rec = self.recognizer.init_hidden().unsqueeze(0).expand(batch_size, -1, -1)

    def parameters(self):
        return list(self.agent.hypernet.parameters())
        # return list(self.agent.hypernet.embed.parameters())
    ...
    def load_models(self, path):
        self.agent.hypernet.load_state_dict(th.load("{}/agent.th".format(path), map_location=lambda storage, loc: storage), strict=True)
