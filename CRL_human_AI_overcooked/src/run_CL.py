...
def run_sequential(args, logger):
    ...
    if args.use_ER:
        replay_buffer = ReplayBuffer(scheme, groups, 12 * 20 * args.batch_size_run, env_info["episode_limit"] + 1,
        ...
        while runner.t_env <= args.t_max * (human_index + 1):
            ...
            if buffer.can_sample(args.batch_size) and off_buffer.can_sample(args.off_batch_size):
                ...
                if args.use_ER and replay_buffer.can_sample(args.batch_size * args.replay_ratio):
                    ...
                    replay_buffer_sample = replay_buffer.uni_sample(int(args.off_batch_size * args.replay_ratio))
    ...
    average_score = 0
    for i in range(12):
        average_score += int(res[11][i])
        if i < 11:
            backward_transfer[i] = int(res[11][i]) - int(res[i][i])
        if i > 0:
            forward_transfer[i] = int(res[i-1][i])
    average_score /= 12
    backward_transfer_mean = sum(backward_transfer) / 11
    forward_transfer_mean = sum(forward_transfer) / 11
...
def find_weight_path(checkpoint_path):
    ...
    for name in os.listdir(checkpoint_path):
        ...
        elif os.path.isdir(full_name) and \
            (name == 'best' or name == 'best_fintune'):
            best_checkpoint_path = full_name
            break
...
def test_CL(args, mac, logger, runner, human_models, cur_human_index, performance_task=None):
    ...
    for j in range(0, len(human_models)):
        ...
        if str(cur_human_index) not in performance_task.keys():
            performance_task[str(cur_human_index)] = {str(j): np.mean(test_rewards)}
        else:
            performance_task[str(cur_human_index)][str(j)] = np.mean(test_rewards)
